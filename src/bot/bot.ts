    ...
    return async (ctx: ContextMessageUpdate): Promise<void> => {
      ...
      await ctx.reply(
        `Hi ${ctx.from.first_name} ${ctx.from.last_name || ''}! 🎉\n` +
          'I am an inline bot for googling *web*, *images* and *YouTube*.\n\n' +
          'Usage:\n\n' +
          '🔎 First of all, type `@Google_itBot ` in any chat, and then type anything you' +
          ...
          'You can also join our news channel @Google\\_itBot\\_feed.',
        this.markup,
      );
    };
  }
